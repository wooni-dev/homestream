...
2. 더블클릭으로 실행 — 폴더 선택 창이 먼저 뜸
3. 영상 폴더 지정 → QR 코드 창이 열림
...
> `SERVE_DIR` 없이 실행하면 폴더 선택 다이얼로그가 먼저 뜨고, 폴더를 고르면 QR 코드 창이 열립니다.  
> `SERVE_DIR`을 지정하면 폴더 선택 없이 바로 QR 코드 창이 뜹니다.
